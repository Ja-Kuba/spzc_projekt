...
class TRW:
    def __init__(self, Pd, Pf, theta0, theta1,
                 status_file='outputs/status.log', detected_file='outputs/detected.log'):
...
class TRWPorts(TRW):
    def __init__(self, Pd, Pf, theta0, theta1,
                 status_file='outputs/status_ports.log', detected_file='outputs/detected_ports.log'):
        super().__init__(Pd, Pf, theta0, theta1, status_file, detected_file)

    def put(self, successful, ip_src, ip_dst, dport):
        key = ip_src
        if key in self.hosts_stats:
            self.update(self.hosts_stats[key], successful, f'{ip_dst}:{dport}')

        else:
            new_host = RemoteHostData(ip_addr=ip_src)
            self.hosts_stats[key] = new_host
            self.update(new_host, successful, f'{ip_dst}:{dport}')
